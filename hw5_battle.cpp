...
public:
    int wizardSpell()
    {
        int addAttack{ itemPair.first == Item::Staff ? itemPair.second : 0 };
        return (rand() % maxAttack) + addAttack;
    }
...
class WizardWarrior : public Warrior, public Wizard
{
public:
    int attack()
    {
...
int main()
{
    ...
    Monster monster;
    WizardWarrior wizardwarrior;
    wizardwarrior.getItem();
    monster.getItem();

    while (true)
    {
        monster.takeDamage(wizardwarrior.attack());
        if (monster.getLife() <= 0)
        {
            std::cout << "WizardWarrior is win!!!";
            return 0;
        }

        wizardwarrior.takeDamage(monster.attack());
        if (wizardwarrior.getLife() <= 0)
        {
